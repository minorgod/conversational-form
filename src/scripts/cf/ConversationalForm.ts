...
import { TagGroup, ITagGroup } from './form-tags/TagGroup'
...
import { TagsParser, DataTag } from './parsing/TagsParser'
...
export class ConversationalForm {
	...
	public getFormData (serialize: boolean = false): FormData | any {
		if (serialize) {
	...
	public remove () {
		...
		ConversationalForm[ this.createId ] = null
	}
...
if (document.readyState === 'complete') {
	...
	setTimeout(() => ConversationalForm.autoStartTheConversation(), 0)
} else {
	// await for when document is ready
	window.addEventListener('load', () => {
		ConversationalForm.autoStartTheConversation()
	}, false)
}
