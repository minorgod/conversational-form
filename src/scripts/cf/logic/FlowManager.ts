...
export interface FlowDTO {
	...
	controlElements?: IControlElement[]
}
...
export default class FlowManager {
	...
	public areConditionsInFlowFullfilled (tagWithConditions: ITag, tagConditions: ConditionalValue[]): boolean {
		...
		for (const tag of this.tags) {
			//const tag: ITag | ITagGroup = value
	...
	public editTag (tag: ITag): void {
		...
		if (this.activeConditions && Object.keys(this.activeConditions).length > 0) {
			...
			for (let i = editTagIndex + 1; i < this.tags.length; i++) {
				const tagToReset: ITag | ITagGroup = this.tags[ i ]
				tagToReset.reset()
			}
		}
	}
