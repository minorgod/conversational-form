...
import {ControlElement, IControlElement, ControlElementEvents, ControlElementVector} from './ControlElement'
...
export class ControlElements {
		...
		public get focus(): boolean {
			...
			const elements: IControlElement[] = this.getElements()
		...
		private tableableRows: IControlElement[][]
		...
		constructor(options: IControlElementsOptions) {
			...
			this.listScrollController = new ScrollController({
				...
				listNavButtons: this.el.getElementsByTagName('cf-list-button') as any,
			})
		}
		...
		public updateStateOnElementsFromTag(tag: ITag) {
			for (const element of this.elements) {
				...
				if((element as any).referenceTag === tag) {
					this.updateStateOnElements(element as any)
					break
				}
			}
		}
			...
			if(this.currentControlElement.type === 'RadioButton') {
				...
				for (const element of this.elements) {
					...
					if(element !== controlElement) {
						...
						(element as RadioButton).checked = true
					}
				}
			...
			if(this.elements && this.elements.length > 0) {
				...
				switch(this.elements[0].type) {
					case 'CheckboxButton' :
						...
						//let values: string[] = []
						for (const element of this.elements) {
							//const element: CheckboxButton = this.elements[i] as CheckboxButton
							if((element as CheckboxButton).checked) {
								if(numChecked++ > 1) {
									break
								}
							}
						...
						for (const element of this.elements) {
							//const el: CheckboxButton = this.elements[i] as CheckboxButton
							if((element as CheckboxButton).checked) {
								if(numChecked > 1) {
									(element as CheckboxButton).partOfSeveralChoices = true
								}

								values.push((element as CheckboxButton).value)
							}

							dto.controlElements.push(element as CheckboxButton)
						}
					...
					case 'RadioButton' :
						for (let i = 0; i < this.elements.length; i++) {
							const element: RadioButton = this.elements[i] as RadioButton
					...
					case 'OptionsList':
						...
						if(dto.controlElements && dto.controlElements[0]) {
							for (let i = 0; i < dto.controlElements.length; i++) {
								const element: IControlElement = dto.controlElements[i] as IControlElement
		...
		public buildTags(tags: ITag[]) {
			...
			if(isElementsOptionsList) {
				this.filterListNumberOfVisible = (this.elements[0] as OptionsList).elements.length
			} else {
		...
		public resize(resolve?: any, reject?: any) {
			...
			if(elements && elements.length > 0) {
				const listWidthValues: number[] = []
				const listWidthValues2: IControlElement[] = []
				...
				const elementsCopyForSorting: IControlElement[] = elements.slice()
				const tabIndexFilteredElements: IControlElement[] = elementsCopyForSorting.sort((a: IControlElement, b: IControlElement) => {
		...
		*/
		...
		private onElementFocus(event: CustomEvent) {
			const vector: ControlElementVector = event.detail as ControlElementVector
		...
		private updateRowColIndexFromVector(vector: ControlElementVector) {
			for (let i = 0; i < this.tableableRows.length; i++) {
				const items: IControlElement[] = this.tableableRows[i]
		...
		private onUserInputKeyChange(event: CustomEvent) {
			...
			const userInput: UserTextInput = dto.dto.input as UserTextInput
			...
			if(this.active) {
				...
				if(shouldFilter) {
					...
					const dto: FlowDTO = (event.detail as InputKeyChangeDTO).dto
		...
		private updateRowIndex(direction: number) {
			...
			if(this.tableableRows[this.rowIndex]) {
				...
				const items: IControlElement[] = this.tableableRows[this.rowIndex]
		...
		private filterElementsFrom(value: string) {
			const inputValuesLowerCase: string[] = value.toLowerCase().split(' ')
			...
			if(elements.length > 1) {
				...
				const itemsVisible: ControlElement[] = []
				for (let i = 0; i < elements.length; i++) {
					const element: ControlElement = elements[i] as ControlElement
					...
					for (let i = 0; i < inputValuesLowerCase.length; i++) {
						const inputWord: string = inputValuesLowerCase[i] as string
		...
		private getElements(): IControlElement[] {
			if(this.elements && this.elements.length > 0 && this.elements[0].type === 'OptionsList') {
				return (this.elements[0] as OptionsList).elements
			}
