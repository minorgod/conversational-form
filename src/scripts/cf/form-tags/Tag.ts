...
export class Tag implements ITag {
		...
		public get rows (): number {
			return this.domElement.hasAttribute('rows') ? parseInt(this.domElement.getAttribute('rows'), 10) : 0
		}
		...
		public get errorMessage(): string {
			if(!this.errorMessages) {
				...
				if(this.domElement.getAttribute('cf-error')) {
					...
					this.errorMessages = Helpers.getValuesOfBars((this.domElement.parentNode as HTMLElement).getAttribute('cf-error'))
		...
		public static testConditions(tagValue: string | string[], condition: ConditionalValue): boolean {
			const testValue = (value: string, conditional: string | RegExp): boolean => {
				...
				// string comparisson
				return tagValue as string === conditional
			}
		...
		protected questions: string[] // can also be set through cf-questions attribute.
		private errorMessages: string[]
		private pattern: RegExp
		private changeCallback: () => void
		private conditionalTags: ConditionalValue[]
		...
		*/
				...
				for (const key in keys) {
					if (keys.hasOwnProperty(key)) {
						...
						if(attr && attr.name && attr.name.indexOf('cf-conditional') !== -1) {
							...
							const conditionalsFromAttribute: string[] = attr.value.indexOf('||') !== -1 ? attr.value.split('||') : attr.value.split('&&')
		...
		protected findAndSetQuestions() {
			...
			if(this.domElement.getAttribute('cf-questions')) {
				...
				const parent: HTMLElement = (this.domElement.parentNode as HTMLElement)
				...
				const forLabel: HTMLElement = document.querySelector("label[for='"+elId+"']") as HTMLElement
		...
		protected findAndSetLabel() {
			...
			if(this.domElement.getAttribute('cf-label')) {
				...
				if(parentDomNode) {
					...
					let labelTags: NodeListOf<Element> | Element[] = (parentDomNode as HTMLElement).tagName.toLowerCase() === 'label' ? [(parentDomNode as HTMLElement)] : (parentDomNode as HTMLElement).getElementsByTagName('label') as any
