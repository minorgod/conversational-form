...
export class Dictionary {
	...
	public static get (id: string): string {
		...
		if (!value) {
			...
			const values: string[] = Helpers.getValuesOfBars(value)
	...
	public static getRobotResponse (tagType: string): string {
		...
		if (!value) {
			...
			const generals: string[] = Helpers.getValuesOfBars(ins.robotData[ 'general' ])
	...
	public static parseAndGetMultiValueString (arr: string[]): string {
		// check ControlElement.ts for value(s)
		let value: string = ''
		for (let i = 0; i < arr.length; i++) {
			const str: string = arr[ i ] as string
			const sym: string = ( arr.length > 1 && i === arr.length - 2 ? Dictionary.get('user-reponse-and') : ', ' )
